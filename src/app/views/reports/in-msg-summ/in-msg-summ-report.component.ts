...
import { in_message_summ } from '../../../material/SummaryReports';
...
@Component({
  selector: 'app-in-msg-summ-report',
  templateUrl: './in-msg-summ-report.component.html',
  styleUrl: './in-msg-summ-report.component.scss'
})
export class InMsgSummReportComponent extends VGCommonModule implements OnInit,AfterViewInit {
  ...
  dataSource: MatTableDataSource<in_message_summ>;
  ...
  dateSubmit() {
    this.dataSource = new MatTableDataSource<in_message_summ>;
    var reqDate = 0;
    ...
    this.messagesService.getInMsgSummReport(this.modifiedDate).subscribe(
      (response) => {
        this.hideloader();
        this.submitted = true;
        console.log(response);
        if (response.error_code == 1100) {
          this.redirectLogin('in-msg-summary-report');
      ...
      (error) => {
        ...
        } else if (this.resp.in_msg_summ.length == 0) {
  ...
  ngOnInit(): void {
    if (this.checkToken('in-msg-summary-report') == false) {
      return;
    }
  }
  ngAfterViewInit() {
