...
import {  out_status_summ } from '../../../material/SummaryReports';
...
@Component({
  selector: 'app-out-status-summ-report',
  templateUrl: './out-status-summ-report.component.html',
  styleUrl: './out-status-summ-report.component.scss'
})
export class OutStatusSummReportComponent extends VGCommonModule implements OnInit, AfterViewInit {
  ...
  dataSource: MatTableDataSource<out_status_summ>;
  ...
  dateSubmit() {
    this.dataSource = new MatTableDataSource<out_status_summ>;
    var reqDate = 0;
    ...
    this.messagesService.getOutStatusSummReport(this.modifiedDate).subscribe(
      (response) => {
        this.hideloader();
        this.submitted = true;
        console.log(response);
        if (response.error_code == 1100) {
          this.redirectLogin('out-status_summary-report');
      ...
      (error) => {
        ...
        } else if (this.resp.out_status_summ.length == 0) {
  ...
  ngOnInit(): void {
    if (this.checkToken('out-status-summary-report') == false) {
      return;
    }
  }
  ngAfterViewInit() {
