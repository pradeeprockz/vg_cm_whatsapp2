...
import { wa_campaign_data_summ } from '../../../material/SummaryReports';
...
@Component({
  selector: 'app-wa-campaign-data-summ-report',
  templateUrl: './wa-campaign-data-summ-report.component.html',
  styleUrl: './wa-campaign-data-summ-report.component.scss'
})
export class WaCampaignDataSummReportComponent extends VGCommonModule implements OnInit, AfterViewInit {
  ...
  dataSource: MatTableDataSource<wa_campaign_data_summ>;
  ...
  dateSubmit() {
    this.dataSource = new MatTableDataSource<wa_campaign_data_summ>;
    var reqDate = 0;
    ...
    this.messagesService.getWaCampaignDataSummaryReport(this.modifiedDate).subscribe(
      (response) => {
        this.hideloader();
        this.submitted = true;
        console.log(response);
        if (response.error_code == 1100) {
          this.redirectLogin('wa-campaign-data-summary-report');
      ...
      (error) => {
        ...
        } else if (this.resp.wa_camp_data_summ_array.length == 0) {
  ...
  ngOnInit(): void {
    if (this.checkToken('wa-campaign-data-summary-report') == false) {
      return;
    }
  }
  ngAfterViewInit() {
